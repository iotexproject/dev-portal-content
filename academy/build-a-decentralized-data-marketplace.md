...
2. Next, each device's owner will be able to register or eventually update their device data stream.

3. Finally, each of these smart devices will produce its unique data stream which can be purchased by third party users. 
...
Let's now look at the logic, line by line.
...
If you'd like to dig deeper into the Pausable and Ownable you can check out  contracts, check out the [Openzeppelin documantation](https://docs.openzeppelin.com/contracts/4.x/).

### Events
Lines 10 to 13 are describing the events that will be emitted appropriately by some of the functions you'll be adding to the contract in just a few minutes. The events will be emitted when a new device is Registered by a device owner, when a registered device is Updated, when a user has Subscribed to a device's data stream, and when a device's owner has Claimed their funds. 

### Mappings
Lines 16 to 19 handle storage. There is an array of bytes32 elements called `deviceIDs` (we'll see how these elements are created soon). The other mappings handle the storage of any authorized device id, registered devices, and data orders. 

### Global Variables
The global variables help to keep track of the registration and subscription fees **set only by the contract owner**. These variables also track the total fees, which comes in handy when the contract owner decides to claim their funds. Also note how the constant `maxDuration` is set to 90 days, establishing the maximum length allowed for a subscription.  

### Structs
There are two custom data types representing a `Device` and a data `Order`. Note that upon registration of a device, the user will be able to input its data frequency `freq`, its price per block `pricePerBlock`(in IoTeX, one block is created every 5 seconds) in terms of IOTX tokens, and more importantly, the two components of the device RSA public key: `rsaPubkeyN` and `rsaPubkeyE`. 

When a new subscriber decides to create an Order for a device, they will use this public key to encrypt the storage endpoint and the storage token. The device, will then be able to decrypt these values with its private key, and will know where to send the data (`storageEPoint`) and the "password" to send it (`storageToken`). 

### Constructor 
The constructor in this case is empty and may be omitted, since, in case of no explicitly defined constructor, a default one will be created by the compiler. 

## Marketplace Owner's Functions
Let's now add the functions that allow the owner to set the device registration fee, set the device subscription fee, and pre-register the allowed devices after manufacturing. Add the following code to your contract: 
...
`setRegistrationFee()`and `setSubscriptionFee` allow to optionally set a fee to be paid to the contract owner by device owners each time they register a device, and data consumers each time they subscribe to a device's data stream. Of course only the contract owner can set these fees.

`preRegisterDevice()` is also exclusive to the contract owner, ad is used to "Authorize" devices, i.e. set which device is actually allowed to be used in the data marketplace (for example, because the contract owner has control over, or trusts, the firmware of these devices only).

To prevent an observer to "steal" the ids of authorized devices before they are assigned to the actual owners, this function expectes the hash of the device id to be provided instead of the actual id (whatever has been chosen as the unique id for these devices).

Every device id hash is coupled with a boolean as an efficient way to check if any hash has already been pre-registered or not and save some gas if it is. 
If the id hash is found as a key inside `allowedIDHashes` and it correspond to a "true" value, then it means it's already been registers and we stp the execution; if not, the mapping is updated with a true value for that hash to mark it as Preregistered. 


## Device owner functions

Let's now cover how a device owner is supposed to **register** a new device. 
...
Device's owners act a "data sellers" in this marketplace and they must call the `Register` to define their data stream settings and, most importantly, to associate their device to their blockchain account. 

This function starts with a series of require statements (lines 15 to 22). Let's dig deeper and see what these statements are actually checking for. The function expects the actual device id to be passed by the device owner and it requires that the hash of the device id is actually inluded in the list of pre-authorized devices (line 15). 

Next, the function requires that the device has not yet been registered, by simply checking that there is no valid device in the Devices mapping corresponding to that id (it's enough to just check for a field like rsaPubkeyN, as if there was an actual device recorded that field would never be "0", as specified by the following requires).
...
After all the requirements are met, the function will finally register the device in the contract (lines 24 to 27). Line 24 updates the registration fee pool by adding the value sent with the transaction. Line 25 sets the `allowedIDHashes` mapping for the device ID to false, to prevent it for being re-registered (as per line 15). 
Line 26 is creating the device object, that is then added to the Devices mapping and can be later retrieved using its corresponding ID. 

Line 27 then "pushes" the device ID to the deviceIDs array comprised of bytes32 elements ((we assume that the chosen device id is no more than 32 bytes, e.g. an IMEI number is 15 bytes). The Devices mapping can then be used to look up a device based on its ID, while the deviceIDs array's Length will be used to quicly tell how many devices have been registered. 
...
The Marketplace contract gives also a device owner the possibility to update their device. Let's look at how this is accomplished. Copy and paste the following code in the contract: 
...
The next thing to do is to create the subscription order - add this code to the function:
```solidity
function subscribe(
    bytes32 _deviceId,
    uint256 _duration,
...
```
The first thing to establish is whether the device the user wants to subscribe to has already had an order. Given the subscription is not active before continuing, in this case it's enough to just update the existing order with the new order data. 
If the device hasn't had any orders yet, then we must set its `hasOrder` property to true, create a new order, and assign it to the orders mapping with the corresponding _deviceId. When a new order is created it will use the current chain height as the starting block for the data stream (line 13).

Now that a new subscription has been created, it's time to update the total fees and balances, so that the claimable amount of both the contract's owner and the device owner are updated appropriately. 

Add this code to the `subscribe()` function:
```solidity
function subscribe(
    bytes32 _deviceId,
    uint256 _duration,
...
A new subscription fee should now be added to the subscription pool (line 10). Now, if the device already has a pending balance, it belongs to the previous subscription and needs to be "archived" into the `settledBalance`, before setting it to the amount paif for this new subscription (i.e. the value of the transaction minus the contract subscription fee). 
...
This function is now completed. Refer to the [repository](https://github.com/iotexproject/pebble-prophecy/tree/main/contracts/contracts) for the complete code. 

## Claiming device data fees
Let's now focus on the next function, which will allow users to claim their balance generated by their devices. 
...
Now, for a device to have a claimable balance, there must have been at least one completed order associated with it (lines 10, 11).

So far so good - let's move to the next bit of logic. 

At this point `pendingBalance` stores the fee relative to the very last order, while `settledBalance` stores all the fees accumulated by any previous subscription. Therefore, we only have to add any pending balance to the `settledBalance` and make sure to reset both balances to "0" before actually performing the transfer (again, to prevent reentrancy attacks):

Add this code to the function to reflect this logic: 
```
function claim(bytes32 _deviceId) public whenNotPaused returns (bool) {
...
It's time for the contract's owner to reap the benefits of the hard work that went into coding this marketplace. Let's look at how to collect marketplace fees:
...
Start by creating a new unsigned integer variable called total, representing the sum of the pooled registration fees and subscription fees. There is a chance that this total may be zero, so it's better to create conditional statement (line 3) to check for that. 
Just like you did earlier, change the value of both `registrationFeeTotal` and `subscriptionFeeTotal` back to zero before inititiating the transfer. 

## View Functions

The logic governing the marketplace is done, but you're not 100% done quite yet: there are a few more functions that would be useful.
It's possible that the marketplace you've created could have to deal with a large number of devices. In this case, retrieving information about all devices at once would not be advisable. A better solution would be to create a pagination mechanism that allows you to query devices in batches. 
...
One more useful function retrieves information on a device given its identifier:
...
and finally, one that provides order details fo a specific device given the device id:
...
# Conclusion 
Considerations about this contract in the context of a fully working data marketplace
This marketplace contract is a theoretical implementation of how real-world data could actually become a tradable good without the need for a centralized party. It would allow users to monetize on their smart devices, and buyers to directly subscribe to the more competitive data provider, based on their needs. 
How interesting would it be to have a truly free market for data? 
